...
本章将会向你展示如何使用数据变形和绘图探索数据。统计学家称之为探索性数据分析或 `EDA`。EDA 实际上是个循环的工作，在 EDA 中你经常需要重复下面的三个环节：
...
```{r histogramcut, echo=FALSE, fig.align='center', fig.cap='cut 变量的分布', out.width='80%', fig.align='center'}
knitr::include_graphics('assets/histogramcut.png')
```

由于这里我设定了 `freq` 选项，所以纵轴显示的是频数。另外你也可以手动计算 cut 变量每个因子的观测值的数目然后绘图：
...
```{r histogramcut2, echo=FALSE, fig.align='center', fig.cap='cut 变量的分布', out.width='80%', fig.align='center'}
knitr::include_graphics('assets/histogramcut2.png')
```
...
```{r histcarat, echo=FALSE, fig.align='center', fig.cap='carat 变量的分布', out.width='80%', fig.align='center'}
knitr::include_graphics('assets/histcarat.png')
```

你可以使用 `width(0.5)` 指定每组组宽为 0.5 ：

```stata 
hist carat, freq width(0.5)
```

```{r histcarat2, echo=FALSE, fig.align='center', fig.cap='carat 变量的分布', out.width='80%', fig.align='center'}
knitr::include_graphics('assets/histcarat2.png')
```

直方图将 x 轴分成等间距的箱，然后使用条的高度显示每个箱子中的观测数。在上图中，最高的条形显示近 3000 个观测值的 carat 变量值介于 0.25 和 0.75 之间，这是条形图的左右边界。

`width()` 选项可以用来设置组宽，该选项是以 x 变量为单位进行测量的。在使用直方图的时候，你应该始终探索各种带宽，因为不同的宽度可以揭示不同的模式，例如图\@ref(fig:histsmallercarat)是我们放大 carat < 3 的钻石并选择较小的宽度绘制的直方图：
...
```{r histsmallercarat, echo=FALSE, fig.align='center', fig.cap='carat < 3 子集中 carat 变量的分布', out.width='80%', fig.align='center'}
knitr::include_graphics('assets/histsmallercarat.png')
```
...
```{r kdensitycarat, echo=FALSE, fig.align='center', fig.cap='carat 变量的核密度分布', out.width='80%', fig.align='center'}
knitr::include_graphics('assets/kdensitycarat.png')
```

## 典型值

条形图和直方图可以展示出变量取哪些值的的情况更多、哪些值的情况较少。
